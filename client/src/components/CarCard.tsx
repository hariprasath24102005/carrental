  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden flex flex-col group h-full border border-ag-border/70 hover:border-ag-cyan/40">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2 py-2 border-y border-ag-border/50 text-xs text-slate-300">
          <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-ag-surface/50">
          ...
          <div className="flex flex-col items-center justify-center p-2 rounded-xl bg-ag-surface/50">
            ...
            <span className="font-semibold text-white">{car.seating_capacity} Seats</span>
        ...
        {/* PRICE & FOOTER ACTION */}
        <div className="pt-2 flex items-center justify-between border-t border-ag-border/40">
          <div>
            <span className="text-xs text-slate-400 uppercase tracking-wider block">Rental Rate</span>
            <div className="flex items-baseline gap-1">
              <span className="font-heading text-2xl font-black text-white">${car.price_per_day}</span>
          ...
          <div className="flex items-center gap-2">
            <Link
              to={`/cars/${car.id}`}
              className="px-3 py-2 rounded-xl bg-ag-surface hover:bg-ag-border text-slate-200 text-xs font-semibold transition-colors"
            >
              Details
            </Link>
            {isAvailable && (
              <Link
                ...
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-ag-cyan to-blue-500 text-slate-950 font-bold text-xs flex items-center gap-1 shadow-md hover:scale-105 transition-transform"
              >
                Book
